    ...
    $submenuListBox.Add_MouseDoubleClick({
        switch ($submenuListBox.SelectedItem) {
            ...
            "nilesoft nss [bk]" {
                nw_powershell -Command "
                    # cd c:\
                    Copy-Item -Path 'C:\Program Files\Nilesoft Shell\shell.nss'  -Destination 'C:\@delta\ms1\asset\nilesoft_shell\shell.nss' -Force -Verbose
                    Copy-Item -Path 'C:\Program Files\Nilesoft Shell\imports'  -Destination 'C:\@delta\ms1\asset\nilesoft_shell\' -Recurse -Force -Verbose
                                         "
            }
            ...
            "Sonarr" {
                nw_pwsh -Command "
                    ...
                    # $su New-Item -ItemType SymbolicLink -Path C:\ProgramData\Sonarr\sonarr.db -Target C:\@delta\msBackups\@mklink\sonarr\sonarr.db -Force -Verbose
                    # Start-Process C:\ProgramData\Sonarr\bin\Sonarr.exe -Verbose
                Write-Host Do it with manual Restore! -ForegroundColor Green && Start-Process 'C:\@delta\msBackups\ARR_timely'
                                         "
            }
            "Radarr" {
                nw_pwsh -Command "
                    ...
                    # $su New-Item -ItemType SymbolicLink -Path C:\ProgramData\Radarr\radarr.db -Target C:\@delta\msBackups\@mklink\radarr\radarr.db -Force -Verbose
            ...
            "Prowlarr" {
                nw_pwsh -Command "
                    ...
                    # $su New-Item -ItemType SymbolicLink -Path C:\ProgramData\Prowlarr\prowlarr.db -Target C:\@delta\msBackups\@mklink\prowlarr\prowlarr.db -Force -Verbose
