...
function renderEntries() {
    ...
    filtered.forEach(e => {
        ...
        div.innerHTML = `
            <div class="entry-header">
                ...
                <div class="entry-actions">
                    <button class="copy-user" title="Copy Username" data-val="${e.u}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg></button>
                    <button class="copy-pass" title="Copy Password" data-val="${e.p}"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="1"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></button>
                    ...
                    <button class="del-btn danger" title="Delete"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/></svg></button>
                </div>
            </div>
            ${Object.keys(e.fields).length ? '<div class="details-toggle">▼ DETAILS ▼</div>' : ''}
            <div class="entry-details hidden">
                ${Object.entries(e.fields).map(([k,v]) => `<div class="detail-row"><span class="detail-label">${k}:</span><span>${v}</span><button class="copy-field" title="Copy" data-val="${v}"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="1"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button></div>`).join('')}
            </div>
        `;
        ...
        div.querySelector('.copy-user').onclick = (ev) => copy(e.u, ev.currentTarget);
        div.querySelector('.copy-pass').onclick = (ev) => copy(e.p, ev.currentTarget);
        
        div.querySelectorAll('.copy-field').forEach(btn => {
            btn.onclick = () => copy(btn.dataset.val, btn);
        });
