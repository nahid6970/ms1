...
class RoundedBorderDelegate(QStyledItemDelegate):
    def __init__(self, parent=None):
        ...
        self.max_values = {2: 0, 3: 0, 4: 0, 5: 0} # Col index -> Max Value
...
class SettingsDialog(QDialog):
    ...
    def get_settings(self):
        return { 'unit': 'MB/s' if self.unit_combo.currentIndex() == 0 else 'Mbps', 'interface': self.if_combo.currentText(), 'win_w': self.win_w.value(), 'win_h': self.win_h.value(), 'row_height': self.row_h.value(), 'col_widths': [sb.value() for sb in self.spins] }
...
class MonitorStats:
    ...
    def update_packet(self, pid, size, direction):
        with self.lock:
            ...
            if direction == 'sent':
                self.proc_data[pid]['sent_total'] += size; self.proc_data[pid]['sent_curr'] += size
            else:
                self.proc_data[pid]['recv_total'] += size; self.proc_data[pid]['recv_curr'] += size
    ...
    def get_snapshot(self):
        ...
        with self.lock:
            ...
            missed_dl = max(0, dl_delta - sum_dl); missed_ul = max(0, ul_delta - sum_ul)
            if missed_dl > 1024 or missed_ul > 1024:
                snap.append({ 'pid': 0, 'name': '[SYSTEM/OTHER/VPN]', 'exe': '', 'dl_speed': missed_dl, 'ul_speed': missed_ul, 'dl_total': missed_dl, 'ul_total': missed_ul })
...
class NetworkThread(threading.Thread):
    ...
    def run(self):
        def cb(p):
            ...
            if IP in p:
                sz = len(p); ip_p = p[IP]; direction = 'sent' if ip_p.src in self.stats.local_ips else 'recv'
...
class App(QMainWindow):
    ...
    def load_settings(self):
        self.win_w = 1000; self.win_h = 700; self.row_height = 40; self.col_widths = [60, 200, 120, 120, 120, 120]; self.current_sort_col = 2; self.current_sort_order = Qt.SortOrder.DescendingOrder; self.unit = "MB/s"; self.interface = str(conf.iface); self.show_dl = True; self.show_ul = True
        if os.path.exists(SETTINGS_FILE):
            try:
                with open(SETTINGS_FILE, 'r') as f:
                    s = json.load(f); self.win_w = s.get('win_w', 1000); self.win_h = s.get('win_h', 700); self.row_height = s.get('row_height', 40); self.col_widths = s.get('col_widths', [60, 200, 120, 120, 120, 120]); self.current_sort_col = s.get('sort_col', 2); self.current_sort_order = Qt.SortOrder(s.get('sort_order', 1)); self.unit = s.get('unit', 'MB/s'); self.interface = s.get('interface', str(conf.iface)); self.show_dl = s.get('show_dl', True); self.show_ul = s.get('show_ul', True)
            except: pass

    def save_settings(self):
        try:
            os.makedirs(SETTINGS_DIR, exist_ok=True)
            settings = { 'unit': self.unit, 'interface': self.interface, 'win_w': self.width(), 'win_h': self.height(), 'row_height': self.row_height, 'col_widths': [self.tree.columnWidth(i) for i in range(6)], 'sort_col': self.current_sort_col, 'sort_order': self.current_sort_order.value, 'show_dl': self.show_dl, 'show_ul': self.show_ul }
    ...
    def init_ui(self):
        self.setStyleSheet(f"""
            ...
            QCheckBox {{ color: {CP_CYAN}; font-weight: bold; }}
        """)
        ...
        self.delegate = RoundedBorderDelegate(self.tree)
    ...
    def update_stats(self):
        ...
        for n, gd in groups.items():
            ...
            r.setForeground(2, QColor(CP_CYAN if gd['dl_s'] > 0 else CP_TEXT)); r.setForeground(3, QColor(CP_ORANGE if gd['ul_s'] > 0 else CP_TEXT))
    ...
    def show_settings(self):
        ...
        if dlg.exec():
            ...
            for i, w in enumerate(self.col_widths): self.tree.setColumnWidth(i, w)
            self.tree.setHeaderLabel(2, f"DL SPEED"); self.tree.setHeaderLabel(3, f"UL SPEED"); self.save_settings(); self.restart_app()
