...
# KEY_ICON_SCALE sizes each icon relative to the app font size; the ban is a
# touch smaller so its full-bleed circle never clips and centers on 'Rule'.
    ...
    def generate_ahk_script(self):
        try:
            ...
            output_lines.extend([
                ...
                "global global_menu_registry := Map()",
                "",
                "OnNativeMenuPopup(wParam, lParam, msg, hwnd) {",
                "    if (global_menu_registry.Has(wParam)) {",
                "        menuObj := global_menu_registry[wParam]",
                "        if (!menuObj.is_populated) {",
                "            menuObj.Populate()",
                "        }",
                "    }",
                "    return 0",
                "}",
                "OnMessage(0x0117, OnNativeMenuPopup)",
                "",
                ...
                "            global_menu_registry[this.Handle] := this",
                "            this.Add(\"...\", (ItemName, ItemPos, MyMenu) => 0)",
                "        }",
                "    }",
                "",
                "    Populate() {",
            ...
            if not self.use_native_menu:
                output_lines.extend([
                    ...
                    "        isMatchingCtrl := (ctrlHwnd == CustomMenuGUI.hoverTargetCtrl)",
                    "        if (!isMatchingCtrl) {",
                    "            for b in CustomMenuGUI.buttons {",
                    "                if (b.ctrl.Hwnd == CustomMenuGUI.hoverTargetCtrl) {",
                    "                    if (HasProp(b, \"arrowCtrl\") && b.arrowCtrl != \"\" && ctrlHwnd == b.arrowCtrl.Hwnd) {",
                    "                        isMatchingCtrl := true",
                    "                    }",
                    "                    break",
                    "                }",
                    ...
                    "                sepOptions := (idx == 1) ? \"w1 h1 Background\" . lineColor : \"y+4 w1 h1 Background\" . lineColor",
                    ...
                    "        OnMessage(0x020A, CustomMenuGUI_MouseWheel)",
                    ...
                    "                }",
                    "                ",
                    "                guiObj.Show(\"x\" . showX . \" y\" . showY . \" w\" . gW . \" h\" . gH)",
                    "            } else {",
                    "                showX := mX - 10",
                    "                showY := mY - 10",
                    "                ",
                    ...
                    "            ctrl := btnObj.ctrl",
                    "            try {",
                    "                shouldHighlight := (idx == CustomMenuGUI.selectedIndex)",
                    "                isHighlighted := HasProp(btnObj, \"isHighlighted\") ? btnObj.isHighlighted : false",
                    "                if (shouldHighlight != isHighlighted) {",
                    "                    btnObj.isHighlighted := shouldHighlight",
                    "                    if (shouldHighlight) {",
                    ...
                    "            CustomMenuGUI.GoBack()",
                    "        } else {",
                    "            CustomMenuGUI.OnItemClick(btnObj.itemIdx, btnObj.menuObj)",
                    "        }",
                    "    }",
                    "",
                    "    static OnItemClick(itemIdx, menuObj) {",
                    "        if (CustomMenuGUI.isTransitioning)",
                    "            return",
                    "        item := menuObj.items[itemIdx]",
                    "        action := item.action",
                    "        ",
                    ...
                    "                action(item.label, itemIdx, menuObj)",
                    "            }",
                    "        }",
                    "    }",
                    "",
                    "    static EnterSubmenu(submenuObj) {",
                    "        if (CustomMenuGUI.isTransitioning || CustomMenuGUI.activeMenu == submenuObj)",
                    "            return",
                    "        CustomMenuGUI.CancelHoverTimer()",
                    "        CustomMenuGUI.isTransitioning := true",
                    ...
                    "    }",
                    "",
                    "    static OnMouseWheel(wParam) {",
                    "        guiObj := CustomMenuGUI.guiObj",
                    "        if (!guiObj)",
                    "            return",
                    "        guiObj.GetPos(,, &gW, &gH)",
                    "        firstBtn := CustomMenuGUI.buttons[1].ctrl",
                    "        lastBtn := CustomMenuGUI.buttons[CustomMenuGUI.buttons.Length].ctrl",
                    "        firstBtn.GetPos(&fx, &fy, &fw, &fh)",
                    "        lastBtn.GetPos(&lx, &ly, &lw, &lh)",
                    "        totalContentHeight := (ly + lh) - fy + 10",
                    "        if (totalContentHeight <= gH)",
                    "            return",
                    "        delta := wParam >> 16",
                    "        if (delta > 32767) {",
                    "            delta -= 65536",
                    ...
                    "            if ((ly + lh) <= gH - 5)",
                    "                return",
                    "            for btnObj in CustomMenuGUI.buttons {",
                    "                btnObj.ctrl.GetPos(&cx, &cy, &cw, &ch)",
                    "                btnObj.ctrl.Move(cx, cy - scrollAmount)",
                    "                if (HasProp(btnObj, \"arrowCtrl\") && btnObj.arrowCtrl != \"\") {",
                    "                    btnObj.arrowCtrl.GetPos(&acx, &acy, &acw, &ach)",
                    "                    btnObj.arrowCtrl.Move(acx, acy - scrollAmount)",
                    "                }",
                    "            }",
                    "        } else {",
                    "            if (fy >= 5)",
                    ...
                    "}",
                    "CustomMenuGUI_MouseWheel(wParam, lParam, msg, hwnd) {",
                    "    return CustomMenuGUI.OnMouseWheel(wParam)",
                    "}",
