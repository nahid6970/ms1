...
class ItemDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.exec_type_combo.addItems(["other", "ahk_v2 (show)", "ahk_v2 (hide)"])
    ...
    def on_exec_type_changed(self, text):
        pass  # No auto-fill: other=direct exe path, ahk_v2=target script path
    ...
    def load_data(self):
        ...
        self.exec_type_combo.setCurrentText(self.item.get("ExecutableType", "other") if self.item.get("ExecutableType", "other") in ["other", "ahk_v2 (show)", "ahk_v2 (hide)"] else "other")
...
class MainWindow(QMainWindow):
    ...
    AHK_DIR = r"C:\@delta\output\startup\ahk"
    ...
    def _make_ahk(self, item):
        ...
        args = item.get("Command", "")
        ext = os.path.splitext(path)[1].lower()
        hide = item.get("ExecutableType") == "ahk_v2 (hide)"
        run_as_admin = item.get("run_as_admin", False)

        if ext == ".ps1":
            exe = r"C:\Program Files\PowerShell\7\pwsh.exe"
            run_args = f'-ExecutionPolicy Bypass -File "{path}"' + (f' {args}' if args else '')
        elif ext in (".bat", ".cmd"):
            exe = r"C:\Windows\System32\cmd.exe"
            run_args = f'/c "{path}"' + (f' {args}' if args else '')
        elif ext == ".py":
            exe = r"C:\Users\nahid\scoop\apps\python312\current\pythonw.exe" if hide else r"C:\Users\nahid\scoop\apps\python312\current\python.exe"
            run_args = f'"{path}"' + (f' {args}' if args else '')
        else:
            exe = path
            run_args = args

        full_cmd = f'{exe} {run_args}'.strip() if run_args else exe

        if run_as_admin:
            run_line = f'Run "*RunAs {full_cmd}"\n'
        else:
            show_opt = ' "Hide"' if hide else ''
            run_line = f'Run "{full_cmd}", ,{show_opt}\n'

        script = f'; AHK v2 helper - auto-generated by startup manager\n{run_line}'
        ahk_path = os.path.join(self.AHK_DIR, f"{item['name']}.ahk")
            ...
            try:
                if should_enable:
                     with winreg.OpenKey(winreg.HKEY_CURRENT_USER, reg_path, 0, winreg.KEY_WRITE) as reg_key:
                        ...
                        if exec_type.startswith("ahk_v2"):
                            ahk_path = self._make_ahk(item)
                            full = f'"{self.AHK_EXE}" "{ahk_path}"'
                        elif item.get("run_as_admin"):
