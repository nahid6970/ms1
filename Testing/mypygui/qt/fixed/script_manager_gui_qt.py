...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Use CustomizeWindowHint instead of FramelessWindowHint for taskbar icon support
        self.setWindowFlags(
        ...
        # Set icon AFTER config is loaded (so colors are available)
        self.setup_icon()
    ...
    def setup_icon(self):
        # Create proper icon from SVG with multiple sizes
        svg_data = f"""<?xml version="1.0" encoding="UTF-8"?>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256">
            <rect width="256" height="256" rx="30" fill="#050505"/>
            <path d="M80 64L16 128l64 64" stroke="#00F0FF" stroke-width="20" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
            <path d="M176 64l64 64-64 64" stroke="#00F0FF" stroke-width="20" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
            <path d="M144 32L112 224" stroke="#FCEE0A" stroke-width="20" stroke-linecap="round" fill="none"/>
        </svg>
        """
        ...
        # Save as .ico file for Windows to use
        try:
            icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "app_icon.ico")
            # Create a 256x256 pixmap for the ico file
            ...
            if renderer.isValid():
                renderer.render(painter)
            painter.end()
            ico_pixmap.save(icon_path, "ICO")
            
            # Load from file for better Windows compatibility
...
if __name__ == "__main__":
    # Set App ID BEFORE creating QApplication
    try:
        myappid = 'scriptmanager.qt.v3.2'
    ...
    # Create SVG icon for application and save as .ico file
    svg_data = f"""<?xml version="1.0" encoding="UTF-8"?>
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256">
        <rect width="256" height="256" rx="30" fill="{CP_BG}"/>
        <path d="M80 64L16 128l64 64" stroke="{CP_CYAN}" stroke-width="20" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
        <path d="M176 64l64 64-64 64" stroke="{CP_CYAN}" stroke-width="20" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
        <path d="M144 32L112 224" stroke="{CP_YELLOW}" stroke-width="20" stroke-linecap="round" fill="none"/>
    </svg>
    """
    
    # Save icon to file for Windows taskbar
    icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "app_icon.ico")
    try:
        ico_pixmap = QPixmap(256, 256)
        ico_pixmap.fill(Qt.GlobalColor.transparent)
        ...
        if renderer.isValid():
            renderer.render(painter)
        painter.end()
        ico_pixmap.save(icon_path, "PNG")  # Save as PNG first (more reliable)
    except:
        pass
    ...
    if os.path.exists(icon_path):
        ...
        # Fallback: create in-memory icon
        app_icon = QIcon()
        for size in [16, 20, 24, 32, 40, 48, 64, 128, 256]:
            pixmap = QPixmap(size, size)
            pixmap.fill(Qt.GlobalColor.transparent)
