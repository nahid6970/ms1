...
document.addEventListener('keydown',e=>{if(e.target.tagName==='INPUT')return;if(e.ctrlKey&&e.key.toLowerCase()==='c'){copySelected();return;}if(e.ctrlKey&&e.key.toLowerCase()==='v'){pasteObject();return;}const map={q:'select',w:'translate',e:'rotate',r:'scale'};if(map[e.key.toLowerCase()])setTool(map[e.key.toLowerCase()]);if(e.key.toLowerCase()==='f')frame();if(e.key==='Delete'||e.key==='Backspace')removeSelected();if(e.key==='1')addObject('box');if(e.key==='2')addObject('sphere');if(e.key==='3')addObject('cylinder');if(e.key==='4')addObject('torus');});
