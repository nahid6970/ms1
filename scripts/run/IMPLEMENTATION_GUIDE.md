...
- **Script Location**: Place related scripts in their own subfolder under `C:\@delta\ms1\scripts\run`.
- **Bookmark Format**: Store as a list of dictionaries to support custom names:
  ```json
  [
    {"path": "C:\\path\\to\\file", "name": "Custom Name"},
    {"path": "C:\\another\\path", "name": ""}
  ]
  ```
...
- **Problem**: `fzf` redirects `stdin`, causing `input()` to raise `EOFError`.
- **Solution**: Open the console device directly:
  ```python
  ...
  ```
- **Cancellation**: Wrap input logic in `try...except KeyboardInterrupt` to handle Ctrl-C gracefully.

## 5. Handling the "Black Window" Issue
...
## 6. Header Toggling (The "Run.py" Style)
