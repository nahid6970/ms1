...
function updateCustomCursor() {
    ...
    if (!customCursor) initCustomCursor();

    const rect = input.getBoundingClientRect();
    const style = window.getComputedStyle(input);

    // Create a mirror element to measure text width/height up to cursor
    const mirror = document.createElement('div');
    document.body.appendChild(mirror); // Append to body to measure
    ...
    // Correctly position mirror relative to the document, not just viewport
    ...
    mirror.style.boxSizing = style.boxSizing; // Important for padding/border calcs

    // Content up to cursor
    const text = input.value;
    const leftText = text.substring(0, input.selectionStart);

    // We add a special character to finding the end position
    const cursorSpan = document.createElement('span');
    cursorSpan.textContent = '|';

    mirror.textContent = leftText;
    mirror.appendChild(cursorSpan);

    // Get the position of the span relative to the viewport
    const spanRect = cursorSpan.getBoundingClientRect();

    // The visual position we want is:
    // (Span's visual pos) - (Scroll amount of text container)
    // But since spanRect is viewport-relative, we need to convert back to document-relative for the absolute div

    // 1. Viewport-relative top of the virtual cursor line
    const viewportCursorTop = spanRect.top - input.scrollTop;
    const viewportCursorLeft = spanRect.left - input.scrollLeft;

    // 2. Convert to Document-relative (Absolute) coordinates
    const absoluteCursorTop = viewportCursorTop + window.scrollY;
    const absoluteCursorLeft = viewportCursorLeft + window.scrollX;

    // Adjust height to match line height more nicely? 
    // spanRect height should be correct line height
    const height = spanRect.height;

    // Hide cursor if text is scrolled out of view (clipping)
    // Check if the calculated cursor position is outside the input's bounding box
    // Viewport relative check is easiest
    if (viewportCursorTop < rect.top || viewportCursorTop + height > rect.bottom) {
        customCursor.style.display = 'none';
        document.body.removeChild(mirror);
        return;
    }

    // Apply to custom cursor
    customCursor.style.display = 'block';
    customCursor.style.top = absoluteCursorTop + 'px';
    customCursor.style.left = absoluteCursorLeft + 'px';
...
document.addEventListener('selectionchange', updateCustomCursor);
document.addEventListener('input', (e) => {
    if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') updateCustomCursor();
});
