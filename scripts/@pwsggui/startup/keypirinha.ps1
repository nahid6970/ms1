...
$Startup_Label = & $csLabel -content "keypirinha"
...
$Startup_CheckBox.IsChecked = [System.IO.File]::Exists([System.IO.Path]::Combine($env:APPDATA, 'Microsoft\Windows\Start Menu\Programs\Startup', 'keypirinha.lnk'))
# Add an event handler for the checkbox
$Startup_CheckBox.Add_Checked({
$shortcutPath = [System.IO.Path]::Combine($env:APPDATA, 'Microsoft\Windows\Start Menu\Programs\Startup', 'keypirinha.lnk')

if (-not (Test-Path $shortcutPath)) {
$WshShell = New-Object -ComObject WScript.Shell
$shortcut = $WshShell.CreateShortcut($shortcutPath)
$shortcut.TargetPath = "C:\Users\nahid\scoop\apps\keypirinha\current\keypirinha.exe"
$shortcut.Save() } })
