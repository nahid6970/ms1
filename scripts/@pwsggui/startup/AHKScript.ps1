...
$Startup_Label = & $csLabel -content "AHK Script"
...
$Startup_CheckBox.IsChecked = [System.IO.File]::Exists([System.IO.Path]::Combine($env:APPDATA, 'Microsoft\Windows\Start Menu\Programs\Startup', 'ahkscripts.lnk'))
# Add an event handler for the checkbox
$Startup_CheckBox.Add_Checked({
$shortcutPath = [System.IO.Path]::Combine($env:APPDATA, 'Microsoft\Windows\Start Menu\Programs\Startup', 'ahkscripts.lnk')

if (-not (Test-Path $shortcutPath)) {
$WshShell = New-Object -ComObject WScript.Shell
$shortcut = $WshShell.CreateShortcut($shortcutPath)
$shortcut.TargetPath = "C:\@delta\ms1\scripts\ahkscripts.ahk"
$shortcut.Save() } })
