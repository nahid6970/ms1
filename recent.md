...
- **Layout SVG icons in menu**: 6 inline SVGs (16x16, dark theme) for BSP/Columns/Rows/VerticalStack/HorizontalStack/Grid rendered via QSvgRenderer → base64 PNG → `<img>` in the rich-action HTML. Cached on first render. Active layout gets cyan text; inactive gets grey. No dot when icon present.
