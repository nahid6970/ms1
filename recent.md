...
- **NEW Komorebi workspace widget** (`KomorebiWidget`): 3 clickable dots + layout label, added in `_build_left` right after the uptime clock. Left-click dot → `komorebic focus-workspace N`. Right-click → menu: change-layout (bsp/columns/rows/vertical-stack/horizontal-stack/ultrawide-vertical-stack/grid/right-main-vertical-stack) + Flip, Toggle Float/Monocle/Pause. Hover tooltip shows per-workspace name, window count (tiled only), layout, ACTIVE/idle. Polls `komorebic state` every 2s in a daemon thread → `_komorebi_queue` → `_drain_komorebi_queue` (300ms timer). **Fast-refresh kick**: every widget action calls `_kick_komorebi_refresh(8s)`; the loop polls 0.3s while kicked, and chunks idle sleep in 0.5s slices so a kick mid-sleep is noticed within ~0.5s. Contents margins (10,0,1,0) match the pagination arrows' padx.
