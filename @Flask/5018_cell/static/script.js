...
function initializeApp() {
    ...
    let markdownMode = localStorage.getItem('markdownPreviewMode');
    // Migration from old boolean storage
...
function highlightSyntax(text) {
    ...
    formatted = formatted.replace(/ŝŝ([\d.]+):(.*?) ŝŝ/g, '<span style="font-weight: bold; -webkit-text-stroke: $1px currentColor; text-stroke: $1px currentColor; paint-order: stroke fill;"><span class="syn-marker">ŝŝ$1:</span>$2<span class="syn-marker"> ŝŝ</span></span>');
    formatted = formatted.replace(/ŝŝ(.*?) ŝŝ/g, '<span style="font-weight: bold; -webkit-text-stroke: 2px currentColor; text-stroke: 2px currentColor; paint-order: stroke fill;"><span class="syn-marker">ŝŝ</span>$1<span class="syn-marker"> ŝŝ</span></span>');
...
function oldParseMarkdownBody(lines, cellStyle = {}) {
    ...
    const formattedLines = codeLines.map(line => {
        ...
        // Sublist: -- item -> ◦ item with more indent (white circle)
            ...
            const content = formatted.replace(/^(\s*)-- /, '');
            formatted = `<span style="display: inline-block; width: 100%; box-sizing: border-box; padding-left: 2em; text-indent: -1em; white-space: pre-wrap;"><span style="display: inline-block; width: 1em; text-indent: 0;">◦</span>${content}</span>`;
        }
        ...
        // Using text-indent for proper hanging indent that preserves tab alignment
            ...
            const content = formatted.replace(/^(\s*)- /, '');
            formatted = `<span style="display: inline-block; width: 100%; box-sizing: border-box; padding-left: 1em; text-indent: -1em; white-space: pre-wrap;"><span style="display: inline-block; width: 1em; text-indent: 0;">•</span>${content}</span>`;
        }

        // Numbered list: 1. item -> 1. item with hanging indent
        if (/^\d+\.\s/.test(originalLine.trim())) {
            const match = originalLine.match(/^(\s*)(\d+\.\s)(.+)$/);
            if (match) {
                const number = match[2];
                // Get the content part from formatted (after the number)
                const content = formatted.replace(/^(\s*)\d+\.\s/, '');
                formatted = `<span style="display: inline-block; width: 100%; box-sizing: border-box; padding-left: 2em; text-indent: -2em; white-space: pre-wrap;"><span style="display: inline-block; width: 2em; text-indent: 0;">${number}</span>${content}</span>`;
            }
        }
    ...
    sheet.columns.forEach((col, index) => {
        ...
        if (index === 0) {
            ...
            toggleSpan.innerHTML =
                `<button onclick="(function(){const i=tableData.sheets.findIndex(s=>String(s.customIndex)==='20260520102127');if(i>=0)switchSheet(i);else showToast('Sheet not found','warning');})()" title="Go to Syllabus" class="btn-header-toggle btn-header-bookmark"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M3 2h7l3 3v9H3V2z"/><path d="M10 2v3h3"/><line x1="5" y1="7" x2="11" y2="7"/><line x1="5" y1="10" x2="11" y2="10"/></svg></button>` +
                `<button onclick="deleteEmptyRows()" title="Delete all empty rows" class="btn-header-toggle"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M2 4h12"/><path d="M5 4V2h6v2"/><path d="M3 4l1 10h8l1-10"/><line x1="6" y1="7" x2="6" y2="11"/><line x1="10" y1="7" x2="10" y2="11"/></svg></button>` +
...
function stripMarkdown(text, preserveLinks = false) {
    ...
    // Only match if it contains fg: or bg: to avoid matching links
    stripped = stripped.replace(/\{(?:fg:[^}]*|bg:[^}]*)\}(.+?)\{\/\}/g, '$1');
...
function convertBanglaToEnglish(event) {
    ...
    const banglaToEnglishMap = [
        ...
        // Decomposed forms (base + nukta ়) - must come before individual consonants
        ['য\u09BC', 'y'], ['ড\u09BC', 'r'], ['ঢ\u09BC', 'rh'],
        ...
        // Nukta fallback (in case of standalone nukta)
        ['\u09BC', ''],
...
function applyTitleTextFormat(event) {
    ...
    if (quickFormatterSelection.isContentEditable) {
        ...
        // Apply the title text syntax
        const formattedText = params.trim() ? `:::${params.trim()}:::${selectedText}:::` : `:::${selectedText}:::`;
        ...
        closeQuickFormatter();
        showToast(`Title style applied`, 'success');
        return;
    }
    ...
    // Apply the title text syntax
    const syntax = params.trim() ? `:::${params.trim()}:::${selectedText}:::` : `:::${selectedText}:::`;
...
function getSyntaxReplaceResult(replacePattern, capturedContent, side, allGroups = []) {
    if (!replacePattern) {
        if (side === 'left') return capturedContent;
        if (side === 'right') return capturedContent;
        return allGroups.length > 0 ? allGroups.join('') : capturedContent;
    }
    
    let result = replacePattern;
    const hasPlaceholder = /text/i.test(replacePattern);
    
    // Support numbered placeholders text1...text9
    if (allGroups.length > 0) {
        for (let i = 0; i < allGroups.length; i++) {
            const num = i + 1;
            const regex = new RegExp(`text${num}`, 'gi');
            result = result.replace(regex, allGroups[i]);
        }
    }
    
    // Support plain 'text' (matches first group)
    result = result.replace(/text/gi, capturedContent);
    
    // Side-specific logic (only for single group for now to avoid template ambiguity)
...
function generateSyntaxReplacePreview(cellContent, findPattern, replacePattern, side = 'both') {
    ...
    const capturedContent = groups[0] || '';
    const before = firstMatch[0];

    const after = getSyntaxReplaceResult(replacePattern, capturedContent, side, groups);
...
function applySyntaxReplace() {
    ...
    let count = 0;
    const newContent = cellContent.replace(regex, (match, ...args) => {
        count++;
        // args = [p1, p2, ..., offset, string]
        const groups = args.slice(0, -2);
        return getSyntaxReplaceResult(replacePattern, groups[0], side, groups);
    });
...
function printCellToPDF() {
    ...
    const headerHtml = `
        <div style="margin-bottom: 20px; padding-bottom: 10px; border-bottom: 2px solid #333; display: flex; justify-content: space-between; align-items: flex-end;">
            <div>
                <h1 style="margin: 0; font-size: 18px; color: #333;">${sheetName}</h1>
                <div style="font-size: 12px; color: #666;">Column: <strong>${columnName}</strong> | Row: <strong>${rowIndex + 1}</strong></div>
            </div>
    ...
    // Footer for the print view
    const footerHtml = `
        <div style="margin-top: 40px; padding-top: 10px; border-top: 1px solid #eee; font-size: 10px; color: #999; text-align: center;">
            Generated by Excel-like Data Table
        </div>
    `;
    ...
    const html = `
        ...
        <head>
            ...
            <style>
                ...
                /* Preserve all styling from the app */
        ...
        <body>
            ...
            <script>
                ...
                window.onload = function() {
                    setTimeout(() => {
                        window.print();
                        // Close/Remove after printing
                        // We use a longer timeout for the removal to ensure the print dialog is handled
                        window.focus();
                    }, 500);
                };
                
                // Remove iframe when window gets focus back (after print dialog closes)
