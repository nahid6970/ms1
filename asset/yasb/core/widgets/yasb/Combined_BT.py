...
#! Example
    ...
    #        subprocess.Popen('cmd /c start C:/@delta/ms1/scripts/mypygui_import/app_store.py')
    #     elif event.button() == Qt.MouseButton.RightButton:
    #        subprocess.Popen('cmd /c start C:/@delta/ms1/scripts/mypygui_import/applist.py')
...
#! Step 5
    def _Find_files_action(self, event: QMouseEvent, label: HoverLabel):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            if modifiers == Qt.KeyboardModifier.ControlModifier:
                ...
                subprocess.Popen(['cmd.exe', '/c', 'start', 'C:\\@delta\\ms1\\scripts\\mypygui_import\\edit_files.py'])
    ...
    def _get_info(self, event, label):
        if event.button() == Qt.MouseButton.LeftButton:
           subprocess.Popen(['cmd.exe', '/c', 'start', 'C:\\@delta\\ms1\\scripts\\info.py'])
        elif event.button() == Qt.MouseButton.RightButton:
           subprocess.Popen(['cmd.exe', '/c', 'code', 'C:\\@delta\\ms1\\scripts\\info.py'])


    def _Tools_yasb(self, event, label):
        if event.button() == Qt.MouseButton.LeftButton:
            subprocess.Popen('cmd /c start C:/@delta/ms1/scripts/mypygui_import/tools.py')
        elif event.button() == Qt.MouseButton.RightButton:
    ...
    def _folder_action(self, event, label):
        if event.button() == Qt.MouseButton.LeftButton:
            subprocess.Popen('cmd /c C:/@delta/ms1/scripts/mypygui_import/folder.py')
        elif event.button() == Qt.MouseButton.RightButton:
            subprocess.Popen('cmd /c Code C:/@delta/ms1/scripts/mypygui_import/folder.py')
    ...
    def _color_pallet_action(self, event: QMouseEvent, label: HoverLabel):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            if modifiers == Qt.KeyboardModifier.ControlModifier:
                subprocess.Popen('cmd /c Code C:/@delta/ms1/scripts/color/color_picker.py')
            else:
                subprocess.Popen('cmd /c C:/@delta/ms1/scripts/color/color_picker.py')
        elif event.button() == Qt.MouseButton.RightButton:
            if modifiers == Qt.KeyboardModifier.ControlModifier:
                subprocess.Popen('cmd /c Code C:/@delta/ms1/scripts/color/color_pallet_rand_fg_bgFF00.py')
            else:
                subprocess.Popen('cmd /c C:/@delta/ms1/scripts/color/color_pallet_rand_fg_bgFF00.py')
        elif event.button() == Qt.MouseButton.MiddleButton:
            self.callback_middle()
    ...
    def PowerToys_Mouse_Pointer_action(self, event, label):
        if event.button() == Qt.MouseButton.LeftButton:
           subprocess.Popen('cmd /c python C:/@delta/ms1/scripts/HotKeys.py powertoys_mouse_crosshair')
        elif event.button() == Qt.MouseButton.RightButton:
           subprocess.Popen('cmd /c python C:/@delta/ms1/scripts/HotKeys.py x_mouse_enable')


    def PowerToys_Text_Extract_action(self, event, label):
        if event.button() == Qt.MouseButton.LeftButton:
           subprocess.Popen('cmd /c python C:/@delta/ms1/scripts/HotKeys.py powertoys_TextExtract')


    def PowerToys_Screen_Ruler_action(self, event, label):
        if event.button() == Qt.MouseButton.LeftButton:
           subprocess.Popen('cmd /c python C:/@delta/ms1/scripts/HotKeys.py powertoys_ruler')


    def PowerToys_Screen_Color_action(self, event, label):
        if event.button() == Qt.MouseButton.LeftButton:
           subprocess.Popen('cmd /c python C:/@delta/ms1/scripts/HotKeys.py powertoys_color_picker')
