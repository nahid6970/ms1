...
if ($PSVersionTable.PSVersion -ge '7.4') {
  function TabExpansion2 {
    ...
    if ($function:TabExpansion) { # Legacy function present: invoke it first.
      ...
      $lastWord = & { # the token immediately preceding the cursor position, if any.
        # NOTE:
      ...
      if ($result) {
        ...
        # Going backward from the current cursor position, find the previous whitespace,
        # so as to determine the start of the token being completed.
        $ndx = $inputScript.Substring(0, $cursorColumn).LastIndexOf(' ')
        $replacementLength = [math]::Max($cursorColumn - $ndx - 1, 0)
        ...
        return [System.Management.Automation.CommandCompletion]::new(
          ...
          $cursorColumn - $replacementLength, # the start index of the token being completed.
          $replacementLength # the length of the token being completed.
        )
      }
