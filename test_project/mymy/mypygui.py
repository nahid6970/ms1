...
# Create a button to show/hide the tools window
tools_window = None
tools_visible = False
...
def toggle_tools_window():
    ...
    if tools_visible:
        ...
        if tools_window:
            ...
            tools_window = None
        tools_visible = False
        ToolsWindow_bt.configure(text="\ueb51")  # Change icon to closed state
    else:
        # Show the tools window
        create_tools_window()
        tools_visible = True
        ToolsWindow_bt.configure(text="\ueb52")  # Change icon to open state

def create_tools_window():
    global tools_window
    ...
    tools = [
        ...
        ("Rclone", "\uef2c", lambda: subprocess.Popen(["cmd /c start C:\\@delta\\ms1\\scripts\\rclone\\RCLONE-MANAGER\\Rclone-Manager.py"], shell=True)),
    ...
    for i, (name, icon, command) in enumerate(tools):
        ...
        if name == "Tools":
            ...
            tool_btn.bind("<Button-3>", lambda e: subprocess.Popen(["cmd /c code C:\\@delta\\ms1\\scripts\\rclone\\RCLONE-MANAGER\\Rclone-Manager.py"], shell=True))
        elif name == "Folder":
...
ToolsWindow_bt.pack(side="left", padx=(2, 0), pady=(1, 0))
ToolsWindow_bt.bind("<Button-1>", lambda event: toggle_tools_window())
